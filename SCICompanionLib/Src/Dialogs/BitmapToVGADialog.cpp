    ...
    case PaletteAlgorithm::Quantize:
        {
            if (_pbmpCurrent)
            {
                ...
                if (imageData)
                {
                    ...
                    std::unique_ptr<uint8_t[]> sciBits = QuantizeImage(imageData.get(), cx, cy, referencePalette.Colors, tempPalette->Colors, _transparentColor, excludeTransparentColorFromPalette);

                    if (performDither)
                    {
                        // This is expensive, and we only need to do this if we're dithering, as sciBits already contains the quantized image data.
                        // Make a copy if the palette generated by QuantizeImage, but with the freeslot information from referencePalette.
                        // Except RGBToPalettized assumes 0x1 for free slots and QuantizeImage assumed 0x0. So flip the bit:
                        PaletteComponent ditherRefPalette = *tempPalette;
                        for (int i = 0; i < ARRAYSIZE(ditherRefPalette.Colors); i++)
                        {
