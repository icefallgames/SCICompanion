...
#define CRYSTAL_TIMER_MATCHINGBRACE 1002
...
private:
    ...
    // Brace matching
    CPoint m_ptMatchingBrace;
    BOOL m_bShowBrace;
    UINT_PTR m_nMatchingBraceTimer;
    ...
    void DrawLineHelperImplPre(CDC *pdc, CPoint &ptOrigin, const CRect &rcClip,
...
protected:
    CExtScrollBar *_pwndScrollBarH, *_pwndScrollBarV;
    ...
    int TextToClientLine(int iLine);
    ...
    int _CalcActualLineLength(int nLineIndex, int nTo);
    ...
    char GetTextChar(const CPoint &ptStart, const CPoint &ptEnd);
...
public:
    ...
    virtual int GetParenBalance(int nLineIndex) { return 0; }
    int BraceDirection(char cTarget);
    BOOL MovePoint(CPoint &pt, int Direction);
...
public:
    ...
    BOOL HighlightBrace(const CPoint &ptStartPos);
    void RemoveBrace();
