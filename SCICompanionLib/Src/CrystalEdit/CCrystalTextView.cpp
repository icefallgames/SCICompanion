...
CCrystalTextView::~CCrystalTextView()
{
    ...
    // Prof-UIS:
    /*
    delete _pwndScrollBarH;
    delete _pwndScrollBarV;*/
}
...
int CCrystalTextView::_CalcActualLineLength(int nLineIndex, int nTo)
{
	//	Actual line length is not determined yet, let's calculate a little
	int nActualLength = 0;
    int nLength = (nTo == -1) ? GetLineLength(nLineIndex) : nTo;
	if (nLength > 0)
	{
...
int CCrystalTextView::GetLineActualLength(int nLineIndex)
{
    ...
    int nActualLength = _CalcActualLineLength(nLineIndex, -1);
...
void CCrystalTextView::DrawLineHelperImplPre(CDC *pdc, CPoint &ptOrigin, const CRect &rcClip,
									 LPCTSTR pszChars, int nOffset, int nCount, CPoint ptTextPos, int nTotalCount)
{
    if (m_bShowBrace && (ptTextPos.y == m_ptMatchingBrace.y))
    {
        int nBraceOffset = m_ptMatchingBrace.x;
        if (nBraceOffset < 0)
        {
            nBraceOffset = 0;
        }

        if (nBraceOffset >= nOffset)
        {
            // Then there is a brace in this chunk of text
            // Draw part before brace:
            if (nBraceOffset > nOffset)
            {
                DrawLineHelperImpl(pdc, ptOrigin, rcClip, pszChars, nOffset, min((nBraceOffset - nOffset), nCount));
            }
            // Draw the brace
            ...
            {
                DrawLineHelperImpl(pdc, ptOrigin, rcClip, pszChars, nBraceOffset + 1, nOffset + nCount - (nBraceOffset + 1));
            }
        }
        else
        {
									 ...
									 LPCTSTR pszChars, int nOffset, int nCount, CPoint ptTextPos)
{
	if (nCount > 0)
	{
		...
		{
            // Try to calculate the position at which selection begins.
            // nSelBegin is an offset from nOffset
			...
			if (nSelBegin > 0)
			{
				DrawLineHelperImplPre(pdc, ptOrigin, rcClip, pszChars, nOffset, nSelBegin, ptTextPos, nCount);
			}
			...
			if (nSelEnd < nCount)
			{
				DrawLineHelperImplPre(pdc, ptOrigin, rcClip, pszChars, nOffset + nSelEnd, nCount - nSelEnd, ptTextPos, nCount);
			}
		...
		{
			DrawLineHelperImplPre(pdc, ptOrigin, rcClip, pszChars, nOffset, nCount, ptTextPos, nCount);
		}
	}
}
...
BOOL CCrystalTextView::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
    ...
    if (abs(_zDeltaMouseWheel) >= WHEEL_DELTA)
    {
        ...
        int nLineCount = GetLineCount();
        int nNewTopLine = fForward ? (m_nTopLine - 3) : (m_nTopLine + 3);

	    if (nNewTopLine < 0)
		    nNewTopLine = 0;
	    if (nNewTopLine >= nLineCount)
		    nNewTopLine = nLineCount - 1;
	    ScrollToLine(nNewTopLine, FALSE);
    }
...
int CCrystalTextView::TextToClientLine(int iLine)
{
    return (iLine - m_nTopLine) * GetLineHeight();
}
...
char CCrystalTextView::GetTextChar(const CPoint &ptStart, const CPoint &ptEnd)
{
    CString text;
...
int CCrystalTextView::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
    ...
    // For prof-uis themed scrollbars:
    ////////////////////// 
    // Note: due to the funky "zero init" thing in CCrystalTextView's constructor, we need
    // to allocate these.
/*    _pwndScrollBarH = new CExtScrollBar();
    _pwndScrollBarV = new CExtScrollBar();
    _pwndScrollBarH->m_eSO = CExtScrollBar::__ESO_BOTTOM;
    _pwndScrollBarV->m_eSO = CExtScrollBar::__ESO_RIGHT;
    if (!_pwndScrollBarV->Create(WS_CHILD | WS_VISIBLE | SBS_VERT | SBS_RIGHTALIGN, CRect(0,0,0,0), this, 1))
    {
        ASSERT(FALSE);
        return -1;
    }
    if (!_pwndScrollBarH->Create(WS_CHILD | WS_VISIBLE | SBS_HORZ | SBS_BOTTOMALIGN, CRect(0,0,0,0), this, 2))
    {
        ASSERT(FALSE);
        return -1;
    }
    _pwndScrollBarH->SyncReservedSpace(_pwndScrollBarV);
    _pwndScrollBarV->SyncReservedSpace(_pwndScrollBarH);
*/
	return 0;
}
...
BOOL CCrystalTextView::HighlightBrace(const CPoint &ptStartPos)
{
	ASSERT_VALIDTEXTPOS(ptStartPos);
    InvalidateLines(ptStartPos.y, ptStartPos.y);

	m_ptMatchingBrace = ptStartPos;
    m_bShowBrace = TRUE;
    m_nMatchingBraceTimer = SetTimer(CRYSTAL_TIMER_MATCHINGBRACE, 2000, NULL);
	return TRUE;
}

void CCrystalTextView::RemoveBrace()
{
    // If we have a matching brace, remove the highlight now
    if (m_bShowBrace)
    {
        m_bShowBrace = FALSE;
        InvalidateLines(m_ptMatchingBrace.y, m_ptMatchingBrace.y);
        KillTimer(m_nMatchingBraceTimer);
    }
}
...
//
// Returns FALSE if it could not move the point in the right direction
//
BOOL CCrystalTextView::MovePoint(CPoint &pt, int Direction)
{
    if (Direction < 0)
    {
        // Move left
    ...
    {
        // Move right.
        if (pt.x == GetLineLength(pt.y))
        {
            if (pt.y < GetLineCount() - 1)
            {
                pt.y++;
                pt.x = 0;
            }
...
int CCrystalTextView::BraceDirection(char cTarget)
{
    // Backward direction
    ...
    else if(cTarget == '{' || cTarget == '(' || cTarget == '[')
    {
        return 1;
    }
    return 0;
...
/*
CScrollBar *CCrystalTextView::GetScrollBarCtrl(int nBar) const
{
    // For prof-uis themed scrollbars:
    ASSERT_VALID( this );
    ...
    if (nBar == SB_HORZ)
    {
        if (_pwndScrollBarH->GetSafeHwnd() != NULL)
        {
            return (const_cast<CExtScrollBar*>(_pwndScrollBarH));
        }
    }
    else
    {
        if (_pwndScrollBarV->GetSafeHwnd() != NULL)
        {
            return (const_cast<CExtScrollBar*>(_pwndScrollBarV));
        }
    }
    return NULL;
}
