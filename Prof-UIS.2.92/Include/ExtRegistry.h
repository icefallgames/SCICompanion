...
public:
		...
		)
	{
		...
		if( hSubKey )
		{
				...
				)
			{
				...
				{
					if( RegQueryValueEx(
							hSubKey, LPCTSTR(pszValName), 0, NULL,
		...
		)
	{
		...
		{
			DWORD dwType, dwRealLength;
			if (RegQueryValueEx( hSubKey, LPCTSTR(pszValName), 0, &dwType, NULL, &dwRealLength) == ERROR_SUCCESS)
			{
				if( dwType == REG_SZ
					|| dwType == REG_EXPAND_SZ
					|| dwType == REG_LINK
	...
	{
		return RegLoadNewString(hKey, LPCTSTR(NULL), LPCTSTR(pszValName), pszString, pdwLength);
	}
	...
	{
		return RegLoadString(hKey, LPCTSTR(NULL), LPCTSTR(pszValName), pszString, dwLength);
	}
		...
		)
	{
		...
		if (hSubKey)
		{
			DWORD dwType, dwSize;
			if( RegQueryValueEx(
					hSubKey, LPCTSTR(pszValName), 0, &dwType,
					NULL, &dwSize
					) == ERROR_SUCCESS
				)
			{
	...
	{
		return RegLoadNumber(hKey, LPCTSTR(NULL), LPCTSTR(pszValName), pdwNumber);
	}
	bool LoadNumber(__EXT_MFC_SAFE_LPCTSTR pszSubKey, __EXT_MFC_SAFE_LPCTSTR pszValName, DWORD *pdwNumber)
	{
		return RegLoadNumber(hKey, LPCTSTR(pszSubKey), LPCTSTR(pszValName), pdwNumber);
	}
	bool SaveNumber(__EXT_MFC_SAFE_LPCTSTR pszValName, DWORD dwNumber)
	{
		return RegSaveNumber(hKey, LPCTSTR(NULL), LPCTSTR(pszValName), dwNumber);
	}
	bool SaveNumber(__EXT_MFC_SAFE_LPCTSTR pszSubKey, __EXT_MFC_SAFE_LPCTSTR pszValName, DWORD dwNumber)
	{
		return RegSaveNumber(hKey, LPCTSTR(pszSubKey), LPCTSTR(pszValName), dwNumber);
	}
		...
		)
	{
		...
		HKEY hSubKey =
			pszSubKey ?
				RegOpen(hKey, LPCTSTR(pszSubKey), KEY_READ) : hKey;
		if( hSubKey )
		{
			DWORD dwType, dwRealSize;
			if( RegQueryValueEx(
					hSubKey, LPCTSTR(pszValName), 0, &dwType, NULL,
					&dwRealSize
					) == ERROR_SUCCESS
				)
			{
		...
		)
	{
		return
			RegSaveBinary(
				hKey, LPCTSTR(NULL), LPCTSTR(pszValName),
				pbyteData, dwSize
				);
	}
	bool SaveBinary(
	...
	{
		return RegDeleteKey(hKey, LPCTSTR(NULL), LPCTSTR(pszValName));
	}
	bool DeleteKey(
		__EXT_MFC_SAFE_LPCTSTR pszSubKey, __EXT_MFC_SAFE_LPCTSTR pszValName)
	{
