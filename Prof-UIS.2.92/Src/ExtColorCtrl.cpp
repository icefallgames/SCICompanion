...
void CExtColorCtrl::cell_t::OnDraw(
	CDC & dc,
	CPalette * pPalette,
	bool bFocused
	)
...
#ifdef _DEBUG
int nCellCount = (int)m_vCells.GetSize();
	ASSERT( nCellCount > 0 );
#endif // _DEBUG

INT		it = 0,
		itFocus0 = (INT)m_vCells.GetSize(),
		itFocus1 = (INT)m_vCells.GetSize();
			...
			{ // walk through all cells at one side
				...
				if( !bOnlySizeChanged )
					m_vCells[nNumber].SetCellColor(
						CExtBitmap::stat_GetRGBFromHLSExtend(
							(float) nAngle,
							L,
							1.0F
							)
...
INT it = 0;
	for( ; it < m_vCells.GetSize(); ++it )
		m_vCells[it].OnDraw( dc, pPalette, false );
COLORREF clr = CExtBitmap::stat_RGBtoCMY( m_clr );
	m_vCells[
			GetRValue(clr)
	...
	)
{
	...
	for( int i = 0; i < NUM_RGB_FULL_CELLS; i++ )
	{
		if( !m_vCells[i].HitTest(point) )
			continue;
		COLORREF clrOrg =
			CExtBitmap::stat_RGBtoCMY( m_clr );
		if( i < NUM_RGB_SINGLE_CELLS )
		{
				COLORREF clr =
					RGB(
...
//					0
//					);
//			clr = CExtBitmap::stat_CMYtoRGB( clr );
			if( (nFlags & MK_CONTROL) != 0 )
			{
				clr = RGB(
					GetRValue(clr),
...
IMPLEMENT_DYNCREATE( CExtColorCtrl, CWnd );
IMPLEMENT_CExtPmBridge_MEMBERS( CExtColorCtrl );
...
COLORREF CExtColorCtrl::SetBkColor( COLORREF clrBk )
{ 
	ASSERT_VALID( this );
...
COLORREF CExtColorCtrl::GetBkColor() const
{ 
	ASSERT_VALID( this );
...
CExtSafeString CExtColorCtrl::GetColorModeName( CExtColorCtrl::e_mode_t eMode )
{
CExtSafeString strModeName;
	ASSERT( CExtColorCtrl::MODE_NAMED_MIN <= eMode && eMode <= CExtColorCtrl::MODE_NAMED_MAX );
	switch( eMode )
	{
	case MODE_HEXAGON:
		if( ! g_ResourceManager->LoadString( strModeName, IDS_EXT_COLOR_MODEL_HEXAGON ) )
			strModeName = _T(" basic colors");
		break;
	case MODE_HSL_PANEL:
		if( ! g_ResourceManager->LoadString( strModeName, IDS_EXT_COLOR_MODEL_HSL_PANEL ) )
			strModeName = _T(" hue, saturation and luminance - mixer");
		break;
	case MODE_HSL_BASIC_HUE:
		if( ! g_ResourceManager->LoadString( strModeName, IDS_EXT_COLOR_MODEL_HSL_BASIC_HUE ) )
			strModeName = _T(" hue, saturation and luminance - basic hue");
		break;
	case MODE_HSL_ROLLER:
		if( ! g_ResourceManager->LoadString( strModeName, IDS_EXT_COLOR_MODEL_HSL_ROLLER ) )
			strModeName = _T(" hue, saturation and luminance - roller");
		break;
	case MODE_RGB_CUBE:
		if( ! g_ResourceManager->LoadString( strModeName, IDS_EXT_COLOR_MODEL_RGB_CUBE ) )
			strModeName = _T(" red, green and blue - cube");
		break;
	case MODE_RGB_MIX:
		if( ! g_ResourceManager->LoadString( strModeName, IDS_EXT_COLOR_MODEL_RGB_MIX ) )
			strModeName = _T(" red, green and blue - mixer");
		break;
	case MODE_GRAY_SCALE_MIX:
		if( ! g_ResourceManager->LoadString( strModeName, IDS_EXT_COLOR_MODEL_GRAY_SCALE_MIX ) )
			strModeName = _T(" gray scale mixer");
		break;
	case MODE_CMY_CUBE:
		if( ! g_ResourceManager->LoadString( strModeName, IDS_EXT_COLOR_MODEL_CMY_CUBE ) )
			strModeName = _T(" cyan, magenta and yellow - cube");
		break;
	case MODE_CMY_MIX:
		if( ! g_ResourceManager->LoadString( strModeName, IDS_EXT_COLOR_MODEL_CMY_MIX ) )
			strModeName = _T(" cyan, magenta and yellow - mixer");
		break;
	case MODE_XYZ_R:
		if( ! g_ResourceManager->LoadString( strModeName, IDS_EXT_COLOR_MODEL_XYZ_R ) )
			strModeName = _T(" red axis");
		break;
	case MODE_XYZ_G:
		if( ! g_ResourceManager->LoadString( strModeName, IDS_EXT_COLOR_MODEL_XYZ_G ) )
			strModeName = _T(" green axis");
		break;
	case MODE_XYZ_B:
		if( ! g_ResourceManager->LoadString( strModeName, IDS_EXT_COLOR_MODEL_XYZ_B ) )
			strModeName = _T(" blue axis");
		break;
	case MODE_XYZ_C:
		if( ! g_ResourceManager->LoadString( strModeName, IDS_EXT_COLOR_MODEL_XYZ_C ) )
			strModeName = _T(" cyan axis");
		break;
	case MODE_XYZ_M:
		if( ! g_ResourceManager->LoadString( strModeName, IDS_EXT_COLOR_MODEL_XYZ_M ) )
			strModeName = _T(" magenta axis");
		break;
	case MODE_XYZ_Y:
		if( ! g_ResourceManager->LoadString( strModeName, IDS_EXT_COLOR_MODEL_XYZ_Y ) )
			strModeName = _T(" yellow axis");
		break;
	} // switch( eMode )
	return strModeName;
}

LRESULT CExtColorCtrl::WindowProc(UINT message, WPARAM wParam, LPARAM lParam) 
{
...
void AFXAPI __EXT_MFC_DDX_ColorCtrl( CDataExchange * pDX, INT nIDC, COLORREF & value )
{
HWND hWndCtrl = pDX->PrepareCtrl( nIDC );
	ASSERT( hWndCtrl != NULL );
	ASSERT( ::IsWindow( hWndCtrl ) );
CExtColorCtrl * pWnd = 
		DYNAMIC_DOWNCAST( CExtColorCtrl, CWnd::FromHandle( hWndCtrl ) );
	if( pWnd != NULL )
	{
		ASSERT_VALID( pWnd );
		if( pDX->m_bSaveAndValidate )
			value = pWnd->m_selClrs.m_clrCurrent;
		else
		{
			pWnd->m_selClrs.m_clrCurrent = value;
			pWnd->Invalidate();
		}
	}
}